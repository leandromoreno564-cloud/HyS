...
export default function AuthenticatedLayout({ children, title = '' }) {
    const { auth, flash, notificacionesNoLeidas, unreadNotifications } = usePage().props;
    ...
    // Menú adaptado 100% a las rutas en español de routes/web.php
    const navigation = [
        { 
            name: 'Dashboard', 
            href: '/dashboard', 
        ...
        { 
            ...
            href: '/empresas', 
            icon: Building2, 
            active: currentPath.startsWith('/empresas') 
        },
        { 
            name: 'Inspecciones', 
            href: '/inspecciones', 
            icon: ClipboardCheck, 
            active: currentPath.startsWith('/inspecciones') 
        },
        { 
            name: 'Medidas Correctivas', 
            href: '/medidas-correctivas', 
            icon: AlertTriangle, 
            active: currentPath.startsWith('/medidas-correctivas') 
        },
    ];

    const esAdmin = currentUser?.role === 'admin' || currentUser?.rol_id === 1;
    ...
    if (esAdmin) {
        navigation.push({ 
            ...
            href: '/usuarios', 
            icon: Users, 
            active: currentPath.startsWith('/usuarios') 
        });
    }
    ...
    const countNotificaciones = notificacionesNoLeidas ?? unreadNotifications ?? 0;
    const nombreUsuario = currentUser?.nombre || currentUser?.name || 'Usuario';
    const matriculaUsuario = currentUser?.matricula || currentUser?.license_number;
            ...
            >
                ...
                <div className="px-5 py-3 border-b border-slate-800/80 bg-slate-900/50">
                    <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${esAdmin ? 'bg-indigo-400' : 'bg-emerald-400'}`} />
                        <span className="text-xs font-semibold text-slate-300 uppercase tracking-wider">
                            {esAdmin ? 'Administrador' : 'Inspector Técnico'}
                        </span>
                    </div>
                    {matriculaUsuario && (
                        <span className="text-[11px] text-slate-400 block mt-0.5 font-mono">
                            Mat. {matriculaUsuario}
                        </span>
                    )}
                </div>
                        ...
                        >
                            ...
                            {countNotificaciones > 0 && (
                                <span className="absolute top-1 right-1 w-4 h-4 bg-rose-500 text-white rounded-full text-[10px] font-bold flex items-center justify-center">
                                    {countNotificaciones > 9 ? '9+' : countNotificaciones}
                                </span>
                            )}
                        </Link>
                                ...
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                                        <div className="px-4 py-2 border-b border-slate-100">
                                            <p className="text-xs font-semibold text-slate-900">{nombreUsuario}</p>

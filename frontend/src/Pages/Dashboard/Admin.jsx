    ...
    return (
        <AuthenticatedLayout title="Panel de Control General">
            <div className="space-y-6">
                {/* Header Action Bar */}
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gradient-to-r from-blue-900 to-indigo-900 rounded-3xl p-6 text-white shadow-xl shadow-blue-950/10">
                    <div>
                ...
                {overdueMeasures > 0 && (
                    <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-center justify-between">
                        ...
                        <Link
                            href="/medidas-correctivas?vencidas=1"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
                    <TarjetaEstadistica
                        ...
                        valor={totalCompanies}
                        subtitulo="Establecimientos auditables"
                        icon={Building2}
                        color="blue"
                    />
                    <TarjetaEstadistica
                        titulo="Total de Inspecciones"
                        valor={totalInspections}
                        subtitulo={`${completedInspections} finalizadas • ${inProgressInspections} en curso`}
                        icon={ClipboardCheck}
                        color="emerald"
                    />
                    <TarjetaEstadistica
                        titulo="Medidas Pendientes"
                        valor={pendingMeasures}
                        subtitulo={`${overdueMeasures} vencidas`}
                        icon={AlertTriangle}
                        color={overdueMeasures > 0 ? 'rose' : 'amber'}
                    />
                    <TarjetaEstadistica
                        titulo="Inspectores & Usuarios"
                        valor={totalUsers}
                        subtitulo="Cuerpo técnico matriculado"
                        icon={Users}
                        color="indigo"
                    />
                </div>
                                    ...
                                    <div key={month} className="flex-1 flex flex-col items-center gap-2 group">
                                        ...
                                        <div className="w-full bg-slate-100 rounded-t-xl h-36 flex items-end p-1">
                                            <div
                                                ...
                                                className="w-full bg-gradient-to-t from-blue-600 to-indigo-500 rounded-lg group-hover:from-blue-700 group-hover:to-indigo-600 transition-all duration-300 shadow-xs"
                                            />
                                        </div>
                                            ...
                                            <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    className={`h-full rounded-full ${
                                                        status.includes('Completad') ? 'bg-emerald-500' :
                                                        status.includes('Progreso') ? 'bg-blue-500' :
                                                        status.includes('Borrador') ? 'bg-slate-400' : 'bg-rose-500'
                                                    }`}
                                                />
                                            </div>
                                        </div>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {recentInspections.length > 0 ? (
                                        recentInspections.map((ins) => {
                                            ...
                                            const nombreInspector = ins.inspector?.nombre || ins.usuario?.nombre || ins.user?.name || 'Inspector';
                                            ...
                                            return (
                                                <tr key={ins.id} className="hover:bg-slate-50/80 transition-colors">
                                                    <td className="py-3 font-semibold text-slate-900">
                                                        <Link href={`/inspecciones/${ins.id}`} className="hover:text-blue-600">
                                                            {razonSocial}
                                                        </Link>
